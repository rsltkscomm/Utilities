...
public class PageLevelHtmlReport {
    ...
    public static String generate(List<PageTransaction> pages) {
        ...
        """,
            blueColors[0], blueColors[1], totalPages,
            blueColors[1], blueColors[2], avgTime,
            blueColors[2], blueColors[3], totalAPIs,
            blueColors[3], blueColors[0], totalTime
        ));
        ...
        for (PageTransaction page : pages) {
            ...
            pageBlocks.append(String.format("""
                ...
                """,
                pageIndex, pageIndex, 
                page.pageName, statusColor, page.pageResponseTime,
                progressWidth, statusColor,
                page.networkEntries.size()
            ));
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.progress-fill').forEach(bar => {
        ...
        try (FileWriter fw = new FileWriter(System.getProperty("performanceReportPath"))) {
            fw.write(html);
    ...
    private static String getStatusColor(long responseTime) {
        if (responseTime < 500) return "#059669";  // Green
        if (responseTime < 2000) return "#d97706"; // Orange/amber
        return "#dc2626"; // Red
    }
    
    private static String getApiStatusColor(int status) {
        if (status >= 400) return "#dc2626";  // Red for errors
        if (status >= 300) return "#d97706";  // Amber for redirects
