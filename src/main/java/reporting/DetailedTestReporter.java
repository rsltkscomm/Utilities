...
public class DetailedTestReporter
{
	...
	public static void updateStep(boolean status, TestCase failConstant, TestCase passConstant)
	{
		...
		if (!status && driver instanceof WebDriver)
		{
			try
			{
				LogEntries logs = ((WebDriver) driver).manage().logs().get(LogType.BROWSER);
		...
		addStep(status ? passConstant : failConstant, status ? StepStatus.PASS : StepStatus.FAIL, page, logPath, null);
	}
	...
	private static String saveBrowserLogs(LogEntries logEntries, String stepName)
	{
		try
		{
			...
			try (FileWriter writer = new FileWriter(file))
			{
				...
				{
					writer.write(i++ + ". [" + entry.getLevel() + "] " + new Date(entry.getTimestamp()) + " - " + entry.getMessage() + "\n");
				}
			}
	...
	public static void addStep(TestCase testCase, StepStatus status, Object context, String logFilePath, File harFile)
	{
		synchronized (MUTEX)
		{
			...
			Optional<TestExecution> opt = getReport().getTestExecutions().stream().filter(e -> e.getTestCaseId().equals(testCase.getTestCaseId())).findFirst();
			...
			boolean duplicate = execution.getSteps().stream().anyMatch(s -> Objects.equals(s.getAction(), testCase.getAction()) && Objects.equals(s.getExpectedResult(), testCase.getExpectedResult()));
	...
	public static String encryptScreenshot(Object context)
	{
		try
		{
			...
			{
				byte[] bytes = page.screenshot(new Page.ScreenshotOptions().setType(ScreenshotType.PNG));
	...
	public static String generateHTMLContentFromJson(String unifiedJson)
	{
		...
		String companyLogoUrl = System.getProperty("CompanyLogo", "https://www.resulticks.com/images/logos/resulticks-logo-blue.svg");
		String productLogoUrl = System.getProperty("ProductLogo", "https://run19.resul.io/assets/resulticks-logo-white-391eec89.svg");

		JsonObject root = new JsonParser().parse(unifiedJson).getAsJsonObject();
		...
		// ================= HEADER =================
		html.append("<div class=\"header\">").append("<img alt=\"Company Logo\" src=\"" + companyLogoUrl + "\"/>").append("<h2>Detail Test Report</h2>").append("<img alt=\"Product Logo\" src=\"" + productLogoUrl + "\"/>").append("</div>");

		// ================= ENV RIBBON =================
		html.append("<div class=\"environment-ribbon\">").append("<span><strong>Environment:</strong> ").append(escapeHtml(environment)).append("</span>").append("<span><strong>Browser:</strong> ").append(escapeHtml(browser)).append("</span>")
				.append("<span><strong>Release:</strong> ").append(escapeHtml(release)).append("</span>").append("<span><strong>Execution Date:</strong> ").append(escapeHtml(executionDate)).append("</span>").append("</div>");

		// ================= BACK LINK =================
		html.append("<div style=\"text-align:right; padding:10px;\">").append("<a class=\"back-btn\" onclick=\"showSummaryReport()\">⬅ Back to Summary Report</a>").append("</div>");

		// ================= TABLE + TOOLBAR =================
		html.append("<div class=\"table-container\">").append("<h3>Test Case Results</h3>")

				// Toolbar (search + filter)
				.append("<div class=\"toolbar\">").append("<div style=\"display:flex; gap:15px; flex-wrap:wrap; margin:20px 0;\">")

				.append("<div>").append("<label><b>Search:</b></label>").append("<input id=\"searchInput\" placeholder=\"Search test cases...\" type=\"text\"/>").append("</div>")

				.append("<div>").append("<label><b>Filter by Status:</b></label>").append("<select id=\"statusFilter\">").append("<option value=\"\">All</option>").append("<option value=\"PASS\">Passed</option>")
				.append("<option value=\"FAIL\">Failed</option>").append("<option value=\"SKIPPED\">Skipped</option>").append("</select>").append("</div>")

				.append("</div></div>");

		// ================= TABLE =================
		html.append("<table id=\"testcaseTable\">").append("<thead><tr>").append("<th></th><th>Module</th><th>Test Case ID</th>").append("<th>Description</th><th>Status</th><th>Duration</th>").append("</tr></thead><tbody>");
		...
		for (JsonElement el : details)
		{
			...
			long durationMs = test.has("durationMillis") ? test.get("durationMillis").getAsLong() : 0;

			String duration = durationMs > 0 ? (durationMs / 1000) + "s" : "-";

			String statusIcon = "PASS".equals(status) ? "✅ Passed" : "FAIL".equals(status) ? "❌ Failed" : "⚠️ Skipped";

			// Main row
			html.append("<tr data-test-id=\"tc").append(counter).append("\" data-expanded=\"false\" onclick=\"toggleDetails(this)\">").append("<td class=\"expand-icon\">+</td>").append("<td>").append(escapeHtml(module)).append("</td>").append("<td>")
					.append(escapeHtml(testCaseId)).append("</td>").append("<td>").append(escapeHtml(desc)).append("</td>").append("<td class=\"").append(getStatusClass(status)).append("\">").append(statusIcon).append("</td>").append("<td>")
					.append(duration).append("</td>").append("</tr>");

			// Details row
			html.append("<tr class=\"details-row\" id=\"tc").append(counter).append("-details\" style=\"display:none\">").append("<td colspan=\"6\">").append("<table class=\"step-table\">").append("<tr><th>Action</th><th>Expected Result</th>")
			...
			for (JsonElement s : steps)
			{
				...
				String icon = "PASS".equals(stepStatus) ? "✅" : "FAIL".equals(stepStatus) ? "❌" : "⚠️";

				html.append("<tr>").append("<td>").append(escapeHtml(getAsString(step, "action"))).append("</td>").append("<td>").append(escapeHtml(getAsString(step, "expected"))).append("</td>").append("<td>")
						.append(escapeHtml(getAsString(step, "actual"))).append("</td>").append("<td>").append(icon).append("</td>").append("<td>")
						.append(step.has("screenshot") && !step.get("screenshot").isJsonNull() ? "<img class=\"screenshot\" src=\"" + step.get("screenshot").getAsString() + "\"/>" : "-").append("</td>").append("</tr>");
			}
