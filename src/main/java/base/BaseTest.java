...
/**
 * Project-specific override of the upstream BaseTest that ships with the Utilities dependency. The original implementation attempted to inject an
 * {@link org.testng.ITestContext} into the {@code @BeforeSuite} hook, which is not supported by TestNG's native injection for suite-level
 * configuration methods and resulted in "Native Injection is NOT supported for @BeforeSuite" errors before any tests could start. Copying the
 * implementation locally allows us to keep all existing behaviour while correcting the hook signature.
 */
...
public class BaseTest
{
	...
	public void beforeMethod(String applicationName, String sheetname, Method method)
	{
		...
		try
		{
			...
			ExtentManager.startTest(test.description(), test.testName(), browser);
	...
	private void loadTestData(Method method)
	{
		...
		Map<String, String> appPropertyMap = Map.of("RegressionAccountSetup", System.getProperty("RegressionAccountSetup"), "RegressionAudience", System.getProperty("RegressionAudience"), "RegressionCommunication",
				...
				System.getProperty("Deploymentchecklist"));
	...
	public void tearDown(ITestResult result)
	{
		...
		try
		{
			...
			{
			case ITestResult.SUCCESS -> ExtentManager.passLabel(result.getName());
			...
			case ITestResult.SKIP -> ExtentManager.skipLabel(result.getName());
			}
