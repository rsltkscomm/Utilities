	...
	public class DuplicateDefectChecker {
	    ...
	    private boolean isSimilarFailure(String existingDescription, String newFailureReason) {
	        ...
	        return existingError.equalsIgnoreCase(newError) || 
	               existingDescription.contains(newFailureReason) ||
	               newFailureReason.contains(existingError);
	    }
	    ...
	    public String findBugKeyByFailure(String failureReason, int maxResults) {
	        ...
	        try {
	            // sanitize for JQL (escape quotes and backslashes)
	            String cleaned = failureReason.replace("\\", "\\\\").replace("\"", "\\\"");

	            // If failureReason is long, pick a short representative token (first 200 chars)
	            if (cleaned.length() > 400) {
	                cleaned = cleaned.substring(0, 400);
	            }

	            // Build JQL: search summary or description (you can adjust to search other fields)
	            // We restrict to project + open bugs to reduce false-positives
	            ...
	            // Use your existing searchJira by temporarily adjusting its maxResults.
	            // Create a small wrapper payload to reuse searchJira(JSON) or directly call searchJira(jql).
	            // Your searchJira currently posts jql and maxResults - we'll call it, then return first key.
	            List<JiraIssue> found = searchJira(jql); // searchJira respects maxResults inside method
	            if (found != null && !found.isEmpty()) {
	                return found.get(0).key;
	            }
	        } catch (Exception e) {
