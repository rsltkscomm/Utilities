...
public class PlaywrightSelectUtil extends PlaywrightDropdownUtil implements SelectInterface
{

	private final Page page;

	public PlaywrightSelectUtil(Page page) {
		super(page);
		this.page = page;
	}
	...
	public boolean selectByVisibleText(String locator, String text)
	{
		try
		{
			resolveLocator(locator).selectOption(new SelectOption().setLabel(text));
	...
	public boolean deselectByVisibleText(String locator, String text)
	{
		try
		{
			resolveLocator(locator).evaluate("(el, txt) => [...el.options].forEach(o => { if(o.label===txt) o.selected=false })", text);
	...
	public boolean deselectByValue(String locator, String value)
	{
		try
		{
			resolveLocator(locator).evaluate("(el, val) => [...el.options].forEach(o => { if(o.value===val) o.selected=false })", value);
	...
	public boolean deselectByIndex(String locator, int index)
	{
		try
		{
			resolveLocator(locator).evaluate("(el, i) => el.options[i].selected=false", index);
	...
	public boolean deselectAll(String locator)
	{
		try
		{
			resolveLocator(locator).evaluate("el => [...el.options].forEach(o => o.selected=false)");
	...
	public String getSelectedOption(String locator)
	{
		try
		{
			String value = resolveLocator(locator).evaluate("el => el.selectedOptions[0]?.textContent").toString().trim();

			ExtentManager.passTest("Currently selected option -> <b>" + value + "</b>");
			return value;
		} catch (Exception e)
		{
	...
	public List<String> getAllSelectedOptions(String locator)
	{
		...
		try
		{
			List<?> values = (List<?>) resolveLocator(locator).evaluate("el => [...el.selectedOptions].map(o => o.textContent.trim())");
	...
	public List<String> getAllOptions(String locator)
	{
		...
		try
		{
			List<?> values = (List<?>) resolveLocator(locator).evaluate("el => [...el.options].map(o => o.textContent.trim())");
	...
	public boolean isMultiple(String locator)
	{
		try
		{
			boolean multiple = (Boolean) resolveLocator(locator).evaluate("el => el.multiple");
	...
	public boolean jsSelectAllText(String locator)
	{
		try
		{
			...
			resolveLocator(locator).evaluate("el => { const r=document.createRange(); r.selectNodeContents(el); const s=window.getSelection(); s.removeAllRanges(); s.addRange(r); }");
