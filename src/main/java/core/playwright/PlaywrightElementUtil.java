...
public class PlaywrightElementUtil extends PlaywrightClickUtil implements ElementInterface
{
	...
	public String getCssValue(String locator, String property)
	{
		try
		{
			return resolveLocator(locator).evaluate("(e, p) => getComputedStyle(e).getPropertyValue(p)", property).toString();
		} catch (Exception e)
		{
			return null;
		}
	...
	{
		return resolveLocator(locator).allInnerTexts().stream().collect(Collectors.joining(","));
	}
	...
	public String normalizeText(String input)
	{
		if (input == null)
			return "";
		return input.replace("\u00A0", " ").replaceAll("[\\u200B\\u200C\\u200D\\uFEFF]", "").replaceAll("\\s+", " ").trim();
	}
	...
	public boolean validateInvalidLinks(String pageName)
	{
		...
		try
		{
			...
			ExtentManager.infoTest("Total <a> tags found on <b>" + pageName + "</b> page: " + anchors.size());
			...
			for (ElementHandle anchor : anchors)
			{
				...
				if (url != null && !url.trim().isEmpty() && !url.contains("javascript") && !url.equals("\"\""))
				{
		...
		{
			ExtentManager.failTest("Exception in validateInvalidLinks for page <b>" + pageName + "</b>: " + e.getMessage());
