...
public class SeleniumScrollUtil extends SeleniumBrowserUtil implements ScrollInterface
{
	...
	private WebElement resolve(Object pr)
	{
		...
		if (pr instanceof String)
			return driver.findElement(autolocator(pr.toString()));
		return null;
	}
	...
	public boolean scrollByElementOffset(Object pr, int xOffset, int yOffset)
	{
		...
		{
			WebElement el = resolve(pr);
			((JavascriptExecutor) driver).executeScript("arguments[0].scrollBy(arguments[1],arguments[2]);", el, xOffset, yOffset);
			return true;
		} catch (Exception e)
		{
			return false;
	...
	{
		((JavascriptExecutor) driver).executeScript(exp);
	}
}
