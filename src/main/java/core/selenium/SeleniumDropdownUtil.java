 ...
 */
public class SeleniumDropdownUtil extends SeleniumAssertUtil implements DropdownInterface
{

	WebDriver driver;

	public SeleniumDropdownUtil(WebDriver driver) {
		super(driver);
		this.driver = driver;
	}
		...
		try
		{
			...
			if (!allElements.isEmpty())
			{
				for (int i = 0; i < allElements.size(); i++)
				{
					...
					String attributeStyle = getAttributeStyle(allElements.get(i));
					highlightElement(allElements.get(i));
					String eleText = allElements.get(i).getText().trim();
					removeHighlight(allElements.get(i),attributeStyle);
